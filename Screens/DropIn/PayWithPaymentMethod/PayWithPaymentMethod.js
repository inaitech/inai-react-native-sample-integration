...
const getStoredPaymentMethodId = async () => {
  let storedPaymentMethodId = null;
  try {
    storedPaymentMethodId = await AsyncStorage.getItem(paymentMethodIdStoreKey);
  } catch (error) {
    console.error(error);
  }
...
const payWithPaymentMethod = async (orderId, navigation) => {
  ...
  let storedPaymentMethodId = await getStoredPaymentMethodId();
  if(storedPaymentMethodId){
    InaiCheckout.payWithPaymentMethod(inaiConfig, storedPaymentMethodId).then((response) => {
      Alert.alert(
        "Result",
    ...
    Alert.alert("No Paymnet Method Id Found")
  }
}

const PayWithPaymentMethod = ({ navigation }) => {
  ...
  const initData = async () => {
    ...
    if (generatedOrderId != null) {
      ...
      payWithPaymentMethod(generatedOrderId, navigation);
    } else {
      setShowActivityIndicator(false);
      Alert.alert(
        "Error",
...
export default PayWithPaymentMethod;